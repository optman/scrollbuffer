...
func MakeRange(pos, end int) Range {
...
func (self *Range) Contain(Pos int) bool {
...
func (self *RangeQueue) AddRange(r Range) {
	...
	for e := self.ranges.Front(); e != nil; e = e.Next() {
		...
		if CanRangeMerge(v, r) {
			...
			if nil == self.ranges.InsertBefore(r, e) {
				panic("insert fail")
			}
...
func (self *RangeQueue) Contain(Pos int) bool {
	for e := self.ranges.Front(); e != nil; e = e.Next() {
		v := e.Value.(Range)
		if v.Contain(Pos) {
			return true
